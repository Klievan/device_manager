...
public class FlutterMcuManagerPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, FirmwareUpgradeDelegate  {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let instance = FlutterMcuManagerPlugin(registrar)
    ...
    public func startDFU(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        do {
            ...
            let firmwarePackage = try McuMgrPackage(from: URL(fileURLWithPath: path));
            ...
            let upgradeConfig = FirmwareUpgradeConfiguration(estimatedSwapTime: estimatedSwapTime ?? 0, reassemblyBufferSize: (arguments["reassemblyBufferSize"] as? UInt64) ?? 0, upgradeMode: upgradeMode ?? FirmwareUpgradeMode.testAndConfirm);
            try dfuManager.start(package: firmwarePackage,using: upgradeConfig);
        } catch {
